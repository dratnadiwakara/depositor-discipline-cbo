...
The target `.tex` file is **always** `tracks/<track>/latex/tables_figures.tex`. This file lives at the track's `latex/` root (not under `sections/`) and is `\input{}`-ed from `main.tex` *after* the bibliography. It hosts every table and figure float in the paper; body sections reference them via `\ref{}` only. No fallback. Stop and ask the user if it does not exist.
...
| **Target file** | Always `tracks/<track>/latex/tables_figures.tex`. Stop if it does not exist. |
| **Insertion line** | If user passes a line number, use it (insert *before* that line). Otherwise use **mention-order placement** (see *Mention-order insertion* below): floats in `tables_figures.tex` must appear in the order of their first `\ref{}` mention across section files, so the new float is spliced in at its mention position, not appended at EOF. Fall back to append-before-closing-macros / end-of-file only when the new label is not yet referenced from any section. |
...
### Mention-order insertion (default placement rule)

Project invariant: float blocks in `tables_figures.tex` appear in the exact order the floats are first mentioned in the body text. When no explicit insertion line is given:

1. Build the **mention sequence**: scan the section files in the order they are `\input{}`-ed from `main.tex` and record the first occurrence of every `\ref{tab:...}` / `\ref{fig:...}` (one combined sequence, tables and figures interleaved).
2. Locate the new float's label in that sequence. Identify its **predecessor** (the float mentioned immediately before it) and find that predecessor's block in `tables_figures.tex`.
3. Insert the new block immediately **after the predecessor's block** (after its `\end{table}` / `\end{figure}`).
4. If the new label is mentioned first overall, insert before the first existing float block. If the new label is not referenced from any section yet, append at EOF and tell the user the block must be repositioned once the mention is written.
5. After insertion, verify: extract the block-label order of `tables_figures.tex` and diff against the mention sequence; report any float that is out of order (pre-existing violations included — report, do not silently reorder other floats).
...
1. **Parse the inputs.** Resolve `<track>` from `$ARGUMENTS`. Target file is always `tracks/<track>/latex/tables_figures.tex` — stop if missing. Decide single vs. multi-panel from the number of source paths supplied (or an explicit user request).
...
5. **Decide insertion line:** explicit line number from user, or **mention-order placement** per the *Mention-order insertion* rule (fall back to append-before-closing-macros / end-of-file only for not-yet-referenced labels).
