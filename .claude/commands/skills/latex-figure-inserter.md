...
4. **Target `.tex` file** — always `tracks/<track>/latex/tables_figures.tex`. This file lives at the track's `latex/` root (not under `sections/`) and is `\input{}`-ed from `main.tex` *after* the bibliography. It hosts every table and figure float in the paper; body sections reference them via `\ref{}` only. No fallback. If that file does not exist, stop and ask the user.
...
| **Insertion line** | If user passes a line number, use it (insert *before* that line). Otherwise use **mention-order placement**: project invariant is that float blocks in `tables_figures.tex` appear in the order of their first `\ref{}` mention across section files (in `main.tex` `\input{}` order, tables and figures interleaved in one sequence). Build the mention sequence, find the new label's predecessor float, and insert immediately after the predecessor's block; if the new label is mentioned first overall, insert before the first float block. Fall back to append-before-closing-macros / end-of-file only when the label is not yet referenced from any section (and tell the user the block must be repositioned once the mention is written). After insertion, diff the block order against the mention sequence and report any float out of order (including pre-existing violations — report, do not silently reorder). |
...
1. **Parse the inputs.** Resolve `<track>` from `$ARGUMENTS`. Target file is always `tracks/<track>/latex/tables_figures.tex` — stop if missing. Decide single vs. multi-panel based on the number of image paths the user passed (and any explicit panel request).
