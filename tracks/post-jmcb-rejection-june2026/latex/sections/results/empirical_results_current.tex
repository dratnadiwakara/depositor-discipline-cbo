...
The results confirm all three.  Throughout, we treat the within-bank
composition shift between uninsured and insured deposits---identified under
bank-by-quarter fixed effects---as the anchor result, because it is the
estimate that survives every specification we throw at it in
Section~\ref{sec:robustness}, including controls that allow 2023-stress
exposure characteristics their own time paths.
...
These magnitudes are economically meaningful without being implausibly large.
For the median treated bank (\$340~million in assets), the binary estimate
corresponds to about \$1.3~million in uninsured time deposits withdrawn or not
renewed; aggregated across the 217 high-CECL banks (mean assets
\$746~million), the implied reallocation is on the order of \$0.6~billion.
Set against the deposit-discipline literature, an effect of 7 percent of the
outcome mean from a single disclosure sits comfortably within the range of
estimated uninsured-deposit sensitivities to bank fundamentals
\citep{martinez2001depositors, egan2017deposit}, and is an order of magnitude
smaller than run-driven outflows in 2023 at the failed institutions---as one
would expect from an informational repricing rather than a panic.
...
comparable timing.  The insured series displays a modest dip in the early
pre-adoption quarters; the individual coefficients are statistically
indistinguishable from zero, and the extended-window event studies in
Section~\ref{sec:robustness:pretrends} show that the dip does not extend
backward as a trend, but we flag it rather than pass over it, and we report
joint pre-trend tests over the full 2018--2022 pre-period there.  The absence
of systematic pre-adoption divergence in either series
...
CECL simply depressed aggregate funding at high-exposure institutions.  In
Section~\ref{sec:robustness:stress} we push this design further, reporting a
fully saturated version that includes every lower-order interaction and a
version that allows the 2022Q4 stress-exposure characteristics to shift the
uninsured--insured gap flexibly by quarter; the triple interaction is
essentially unchanged in both.
...
quarterly NIM by $0.005$ percentage points ($p < 0.05$).  In dollar terms, the
annualized interest-expense effect amounts to roughly \$0.3~million per year
for the median treated bank and the ROA effect to about \$0.5~million of
foregone annual net income---small relative to a panic, but a permanent tax
on the franchise imposed by a one-time disclosure.
...
The heterogeneity of these responses across depositor sophistication, and the
robustness of the sophistication split itself, are developed in
