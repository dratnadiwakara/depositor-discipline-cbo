...
\begin{frame}{Do uninsured depositors actually discipline banks?}
  ...
  \begin{itemize}
    ...
    \item<1-> In the data, riskier banks pay higher rates and hold fewer
      uninsured deposits: depositors decline to roll over maturing claims
...
\begin{frame}{Why the causal evidence has been elusive: three problems}
  ...
  \end{itemize}
  \takeaway{The mandatory CECL adoption of 2023 helps me address these
...
\begin{frame}{The experiment: a mandatory disclosure with a predetermined size}
  ...
  \begin{itemize}
    ...
    \item The adoption date was public years in advance. The size of each
      bank's charge was not observable outside the bank until the Call Report
      was posted
...
% =============================================================================
\begin{frame}{Univariate evidence: uninsured money leaves, insured money arrives}
  \spec{Group means, pp of assets; bases 2023Q1 (top) and 2022Q1 (bottom); 206 High-CECL vs 2{,}265 near-zero banks; no controls or FE}
  \centering
...
\begin{frame}[label=fmag]{Magnitudes: a composition shift, not a funding collapse}
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    \rowcolor{pblue!8}
    Post $\times$ High CECL & \tikzmarknode{hl}{$-$0.39**} & $+$0.71** & $-$1.25*** \\
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \node[draw=pred, very thick, rounded corners, inner sep=3pt, fit=(hl)] (box) {};
    \node[draw=pred, fill=pred!6, rounded corners, text width=0.88\textwidth,
          align=left, font=\footnotesize, inner sep=5pt, anchor=north]
          (note) at ([yshift=-2.35cm]current page.center) {%
      \textcolor{pred}{\textbf{The boxed cell is the headline estimate:}} Post
      $\times$ High CECL on uninsured time deposits/assets, bank + quarter FE.
      The next slides take this one specification apart: by deposit type, by
      capital, by maturity, and by where the money went.};
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Discipline lives exactly where the depositor holds a decision}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.34\textwidth}
      \vspace{6mm}
      \colorbox{pgold!18}{\parbox{0.94\linewidth}{\scriptsize\raggedright
...
\begin{frame}[label=fcapital]{The response is concentrated at thinly capitalized banks}
  ...
  \vspace{-1mm}
  \takeaway{Same disclosed charge, no response in the better-capitalized half.
    A given charge is worse news when the buffer absorbing it is thin}
\end{frame}

% =============================================================================
\begin{frame}[label=fcapmat]{Both margins together: thin capital \emph{and} a scheduled exit}
  ...
  \vspace{-1mm}
  \takeaway{The response sits in one cell: banks where the charge was bad news
    \emph{and} depositors had a near-term decision to make}
\end{frame}
...
\begin{frame}[label=fcapmattotal]{The same 2$\times$2 for total uninsured deposits}
  ...
  \vspace{-1mm}
  \takeaway{Same sign in the same cell, but the aggregate is seven times larger
    and mostly operating money: the CD signal is lost in the noise}
\end{frame}
...
\begin{frame}{What I cannot rule out: attention}
  ...
  \begin{itemize}
    ...
    \item Depositors who had just watched two banks fail may have read the
...
\begin{frame}{Takeaways}
  ...
  \begin{enumerate}
    ...
    \item<1-> \textbf{Policy:} the discipline documented here lives in
      time deposits, not in operating accounts. Proposals that insure \$10M
      business accounts target the balances that do not monitor; the
      relevant margin is whether coverage reaches the CD book
  \end{enumerate}
  \vfill
  \begin{center}
